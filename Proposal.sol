...
contract Proposal is Constants, Common {
    ...
    function initializeParameters(uint projectId) external restricted(projectId) {
        privateInitializeParameters(projectId);
    }

    function privateInitializeParameters(uint projectId) private {
        parameters[projectId][keccak256("VoteDuration")].value = 40 seconds;
        parameters[projectId][keccak256("VoteDuration")].min = 86400 seconds; // 1 day
        ...
        parameters[projectId][keccak256("BufferBetweenEndOfVotingAndExecuteProposal")].value = 40 seconds;
        parameters[projectId][keccak256("BufferBetweenEndOfVotingAndExecuteProposal")].min = 86400 seconds;
        ...
        parameters[projectId][keccak256("ExpirationPeriod")].value = 40 seconds;
        parameters[projectId][keccak256("ExpirationPeriod")].max = 86400 seconds;
    ...
    modifier hasMinBalance (uint projectId, address addr) {
        require(contractsManager.checkMinBalance(projectId, addr),
    ...
    function getVotes(uint projectId, uint proposalId, address a) public view returns (uint, uint) {
        ...
        Delegates delegatesContract = Delegates(contractsManager.contracts(projectId, DELEGATES));
    ...
    function cancelProposal(uint projectId, uint proposalId) external {
        require(proposals[projectId][proposalId].endTime <= block.timestamp, "Can't cancel, voting is ongoing");
        ...
        if ((yesVotes + noVotes) == 0) {
            delete proposals[projectId][proposalId];
            emit CancelProposal(projectId, proposalId);
        }
    }
    ...
    function contestProposal(uint projectId, uint proposalId) external hasMinBalance(projectId, msg.sender) {
        ...
        for (uint64 i = 0; i < proposals[projectId][proposalId].nrOfVoters; i++) {
            ...
            if (noVotes * 100 / contractsManager.votingTokenCirculatingSupply(projectId) >= parameters[projectId][keccak256("VetoMinimumPercentage")].value) {
                delete proposals[projectId][proposalId];
    ...
    function neededToContest(uint projectId) external view returns (uint) {
        return parameters[projectId][keccak256("VetoMinimumPercentage")].value * contractsManager.votingTokenCirculatingSupply(projectId) / 100;
    }
    ...
    function getSpamVoters(uint projectId, uint proposalId) external view returns (uint[] memory) {
        ...
        for (uint64 i = 0; i < proposals[projectId][proposalId].nrOfVoters; i++) {
            if (!delegatesContract.checkVotingPower(projectId, proposals[projectId][proposalId].voters[i],
                contractsManager.votingTokenCirculatingSupply(projectId) / parameters[projectId][keccak256("MaxNrOfDelegators")].value)) {
                indexes[index] = i;
                index++;
                if (index == 200) {
                    return indexes;
    ...
    function removeSpamVoters(uint projectId, uint proposalId, uint[] memory indexes) external {
        Delegates delegatesContract = Delegates(contractsManager.contracts(projectId, DELEGATES));
        uint minimum_amount = contractsManager.votingTokenCirculatingSupply(projectId) / parameters[projectId][keccak256("MaxNrOfDelegators")].value;
    ...
    function countDelegatedVotes(uint projectId, address delegatedAddr, uint proposalId, Token tokenContract, Delegates delegatesContract) public view returns (uint delegatedVotes) {
    ...
    function calculateTotalVotingPower(uint projectId, address addr, uint id, Token tokenContract, Delegates delegatesContract) view public returns (uint) {
