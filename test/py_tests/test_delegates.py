...
class DelegatesTests(EthBaseTest):
    ...
    def setUp(self):
        ...
        self.create_test_account(token_contract=self.tokenContract)
        self.create_test_account(token_contract=self.tokenContract)
        self.create_test_account(token_contract=self.tokenContract, token_amount=10)
    ...
    def test_delegate_vote(self):
        ...
        self.assertEqual(self.delegatesContract.delegates(GS_PROJECT_ID, self.accounts[1].address, 0),
    ...
    def test_undelegate_vote(self):
        ...
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[0].address), ZERO_ETH_ADDRESS)
        try:
            self.delegatesContract.delegates(GS_PROJECT_ID, self.accounts[1].address, 0)
            self.fail()
        except Exception as e:
    ...
    def test_delegate_vote_to_an_address_and_than_to_another(self):
        ...
        self.assertEqual(self.delegatesContract.delegates(GS_PROJECT_ID, self.accounts[2].address, 0),
                         self.accounts[0].address)
    ...
    def test_delegate_vote_to_an_address_that_already_delegated_to_another(self):
        ...
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[0].address),
                         self.accounts[1].address)
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[1].address),
                         self.accounts[2].address)
        self.assertEqual(self.delegatesContract.delegates(GS_PROJECT_ID, self.accounts[1].address, 0),
                         self.accounts[0].address)
        self.assertEqual(self.delegatesContract.delegates(GS_PROJECT_ID, self.accounts[2].address, 0),
    ...
    def test_delegate_addresses_undelegate_one_of_them_and_delegate_back(self):
        ...
        self.assertEqual(self.delegatesContract.delegates(GS_PROJECT_ID, self.accounts[3].address, 0),
                         self.accounts[0].address)
        self.assertEqual(self.delegatesContract.delegates(GS_PROJECT_ID, self.accounts[3].address, 1),
                         self.accounts[1].address)
        self.assertEqual(self.delegatesContract.delegates(GS_PROJECT_ID, self.accounts[3].address, 2),
                         self.accounts[2].address)
    ...
    def test_undelegate_all_from_address(self):
        ...
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[2].address), ZERO_ETH_ADDRESS)

        try:
            self.delegatesContract.delegates(GS_PROJECT_ID, self.accounts[3].address, 0)
            self.fail()
        except Exception as e:
            self.assertIn("execution reverted", str(e))
    ...
    def test_undelegate_all_from_address_and_delegate_back(self):
        ...
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[1].address),
                         self.accounts[3].address)
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[2].address),
                         self.accounts[3].address)
    ...
    def test_remove_unwanted_delegates(self):
        ...
        for i in range(1, 13):
            self.tokenContract.transfer(self.eth_account.address, 10 ** 18, private_key=self.accounts[i].key)

        (addresses, indexes) = self.delegatesContract.getUnwantedDelegates(GS_PROJECT_ID, self.accounts[0].address)

        self.delegatesContract.removeUnwantedDelegates(GS_PROJECT_ID, addresses[:10], indexes[:10],
                                                       private_key=self.private_key)

        self.assertEqual(self.delegatesContract.delegates(GS_PROJECT_ID, self.accounts[0].address, 0),
                         self.accounts[3].address)
        self.assertEqual(self.delegatesContract.delegates(GS_PROJECT_ID, self.accounts[3].address, 0),
                         self.accounts[13].address)
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[2].address), ZERO_ETH_ADDRESS)
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[5].address), ZERO_ETH_ADDRESS)
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[6].address), ZERO_ETH_ADDRESS)
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[7].address), ZERO_ETH_ADDRESS)
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[8].address), ZERO_ETH_ADDRESS)
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[9].address), ZERO_ETH_ADDRESS)
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[10].address), ZERO_ETH_ADDRESS)
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[11].address), ZERO_ETH_ADDRESS)
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[12].address), ZERO_ETH_ADDRESS)
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[3].address),
                         self.accounts[0].address)
        self.assertEqual(self.delegatesContract.delegators(GS_PROJECT_ID, self.accounts[13].address),
                         self.accounts[3].address)
